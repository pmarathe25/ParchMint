...
Place focused unit tests beside implementation code. Name colocated contract modules `*_contract_tests.rs`, integration tests under a crate's `tests/`, and platform-backed suites `native_*.rs`. Use descriptive behavior names such as `comparison_accepts_identical_images`. UI reference images belong in `tests/parchmint-ui-verification/references/`; changes require deliberate visual review. The headless driver in `tests/parchmint-ui-driver/` covers cross-boundary user behavior without OS windows; read `tests/parchmint-ui-driver/README.md` before adding or changing an author-flow scenario. When it finds a regression, use its trace, observations, and diagnostics to reproduce the lowest responsible boundary in a focused unit or contract test. Keep an end-to-end scenario only when rendering, focus, event routing, or cross-boundary integration is part of the behavior. No numeric coverage target is documented, so cover changed behavior and regressions directly.
