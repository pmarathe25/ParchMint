...
4. Populate `penpot_screen_ids` and `penpot_component_ids` in `docs/traceability.csv` for every applicable requirement. Record any applicable must-level requirement without an approved design mapping as a blocking issue.
...
Also update `docs/traceability.csv` with the approved design mappings. S10 may return `needs_approval` only when every applicable requirement has a design mapping or a corresponding blocking entry in `open-issues.yaml`.
