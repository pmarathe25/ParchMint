...
impl HarnessTarget {
    pub(crate) fn id(self) -> Id {
        Id::new(match self {
            ...
            Self::GlobalSearchResults => "global-search-results",
            Self::GlobalSearchMatch(index) => return format!("global-search-match-{index}").into(),
            ...
            Self::CreateStyle => "harness.styles.create",
            Self::CreateMetadataField => "harness.metadata.create",
            Self::DeleteMetadataField(index) => {
                return format!("harness.metadata.delete.{index}").into();
            }
            ...
            Self::OverviewAddGroup => "overview-add-group",
