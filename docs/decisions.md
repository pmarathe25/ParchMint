...
| JSON Schema with generated Rust bindings | `parchmint-contracts` owns only the durable JSON boundaries ParchMint uses. A pinned `typify` generates Rust types from those schemas. |
