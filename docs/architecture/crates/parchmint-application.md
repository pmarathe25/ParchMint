...
pub trait GlobalReplacement: Send + Sync {
    fn preview(&self, request: ReplacementRequest)
...
Project undo retains up to 100 complete operations and 64 MiB of inverse data in
memory. A larger inverse moves to a temporary session file. Eviction removes
whole operations, and a new project command clears redo.

```rust
async fn execute_project(command: ProjectCommand) -> Result<ProjectCommandResult> {
    authorize_live_session(&command)?;
    let applied = state.apply(command)?;
    undo.push(applied.undo_entry())?;
    changes.publish(applied.revision(), applied.changed_resources());
    Ok(applied.result())
}
```
...
inverse, calls `apply_composite_project_edit` for open sessions, updates closed
documents, and stages all affected files in one save transaction. It publishes
the new project state only when all in-memory changes succeed. A later save
failure leaves the complete replacement dirty and protected by recovery.
