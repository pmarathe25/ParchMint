...
preference command + expected revision
  ...
  -> if appearance changed, resolve System, Light, or Dark
  -> if appearance changed, publish one numbered theme snapshot

operating-system appearance event while mode is System
  -> update the resolved system appearance
  -> publish one numbered theme snapshot
```
...
`System`; it does not rewrite the preference file.

```rust
fn publish(controller: &AppearanceController) {
    let snapshot = controller.current();
    controller.publisher.publish(snapshot);
}
```

The global dictionary uses the same preference revision as the spelling
service. A preference save failure leaves the previous durable settings in
place and reports the unsaved in-memory change.
