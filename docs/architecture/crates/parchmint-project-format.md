...
search and word-count caches use `.parchmint/cache/`, and workspace layout uses
the platform application-data directory keyed by project ID.
...
A migration first validates the complete old project. When History is
available, the application records a pre-migration checkpoint through
`HistoryStore`. The codec then produces the complete new resource set in memory
and hands it to the normal save path, which writes the set as one transaction.
...
[`parchmint-contracts`](parchmint-contracts.md) defines the JSON annotation
...
- Equivalent values have one byte representation. The encoder leaves an
  unchanged document alone instead of rewriting it only to change formatting.
- Canonical HTML allows only the supported semantic blocks and marks. Scripts,
  ...
  rejected or removed before encoding.
