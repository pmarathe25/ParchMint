...
pub struct FsProjectRepository<F: ProjectFileSystem> {
    files: F,
    codec: Arc<dyn CanonicalCodec>,
...
by `FsAtomicWriter`. The desktop application selects the native filesystem
implementation of `ProjectFileSystem` and `AtomicFileOps`. Tests supply wrappers
that can pause or fail a specific disk operation. The application calls this
crate on a storage worker, so disk access does not block the UI thread.
