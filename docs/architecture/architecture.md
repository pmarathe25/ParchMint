...
Tauri/React/ProseMirror is the selected implementation baseline. Selection does not count as packaged/native acceptance: S20 must prove a packaged launch, bundled asset load, and privileged IPC round trip on Windows, macOS, and Linux, while S55/S60 must use packaged release builds for editor evidence. Failure stops at G20; agents do not substitute another frontend or weaken a product gate.
...
- Appearance and global dictionary: schema-versioned `ApplicationPreferenceStore` outside project history.
- Resolved appearance and ordered cross-window propagation: one process-wide `AppearanceController`.
...
A machine-readable policy committed at S20 defines allowed licenses, advisory thresholds, source/provenance expectations, exception owner/reason/expiry, and required hashes for bundled native libraries, fonts, and spellcheck dictionaries. CI fails on an unreviewed advisory, non-allowlisted license, unexpected provenance/maintainer change, expired exception, or untracked bundled artifact. Lock changes include an SBOM diff.
...
S55 is a two-dimensional gate. It must first compare concrete mechanisms for sharing document/history state across two mounted views, including transaction ordering, undo grouping, simultaneous and composition-sensitive edits, stale-event handling, detach/reattach, and lifecycle recovery. It then compares canonical-projection strategies against every viable shared-state mechanism. S60 starts only after both selections are captured in the editor contract and accepted through the S55 handoff.
...
Every save targets an immutable revision vector:

```text
SaveRevisionVector
├── project_revision
├── document_editor_revisions { document_id: editor_revision }
├── canonical_resource_hashes
└── save_generation
```

`SaveCoordinator` captures the vector, obtains matching immutable editor projections, and persists an idempotent hash-keyed checkpoint intent before canonical replacement. `HistoryStore.checkpoint` is idempotent for that intent. Canonical-through and checkpointed-through revisions are tracked separately; the UI may report `Saved` only when the required history checkpoint matches the canonical revision vector.
...
On restart, the coordinator reconciles canonical resource hashes, checkpoint intents, and history before acknowledging any revision. A crash or retry between canonical replacement and checkpoint completion must finish the same checkpoint exactly once rather than skip or duplicate it.
...
### 15.8 Application preferences and appearance

```rust
trait ApplicationPreferenceStore {
    fn load(&self) -> Result<ApplicationPreferencesSnapshot>;
    fn compare_and_store(&self, expected_generation: u64, next: ApplicationPreferences) -> Result<ApplicationPreferencesSnapshot>;
    fn subscribe(&self, sink: ApplicationPreferenceSink) -> Result<Subscription>;
}
```

The initial adapter uses a schema-versioned, atomic file in the platform application-data directory. Preference writes are serialized process-wide and report success only after durable replacement. This store owns appearance mode and the global dictionary; neither enters project state, project undo, save, recovery, or history.
...
One `AppearanceController` owns `{mode, observed_system_theme, generation, resolved_theme}`. Operating-system events change the resolved theme only while mode is `system`; an explicit Light/Dark selection is durably stored before success is reported. Every new window receives the latest snapshot before first paint, and broadcasts are generation-ordered so stale system or preference events cannot overwrite a newer choice.
...
The first UI deliverable is `delivery/design-reconciliation/<version>/`. Production code must satisfy semantic HTML, accessibility, maintainability, component reuse, application state, and both themes; generated design code is never accepted solely for visual resemblance.
...
- Deny top-level and iframe navigation away from bundled application origins; route user links through a validated external-open platform service.
- Bind each webview to an unforgeable window/project-session capability verified server-side on every privileged command; never trust a caller-supplied `project_id` alone.
- Use opaque, dialog-derived path handles for privileged file operations after project open/create; do not expose generic filesystem, shell, or HTTP capabilities to the webview.
...
S20 commits a threat model and capability matrix and tests remote-navigation rejection, cross-window/project command rejection, path-handle scope, and the packaged CSP/capability manifests.
