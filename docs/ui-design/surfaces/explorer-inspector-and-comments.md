...
- Comments are one continuous list. The header exposes thread status and
  Resolve/Reopen using the shared button style. Collapsed replies remain
  discoverable; expanded replies are chronological above an in-thread composer.
  Orphaned comments show a centered, navigable remediation message.
