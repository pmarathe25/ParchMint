...
**Status:** Final v1 product baseline  
**Version:** 1.0  
**Date:** 2026-07-28  
...
- Word counts.
- Conditional spellcheck, subject to cross-platform correctness and latency.
...
- **WS-010:** Focus, selection, and active context must be visually distinguishable and keyboard accessible.

### 8.3 Explorer and hierarchy

- **TREE-001:** Explorer must show visually separated Manuscript and Research sections.
...
- **FMT-004:** Reserved styles may be visually edited and visibly renamed but may not be deleted.
...
- **TOOL-002:** The toolbar must always be available in Editor mode but may be collapsed.
...
- **TOOL-005:** The toolbar must expose style selection, common inline marks, lists/quote, links, Scene Break, Page Break, and comment creation where appropriate.
...
- **META-004:** Field definitions must have stable IDs, label, optional description, applicability by section/node type, optional default, Card visibility, and display order.
...
- **CMT-002:** A comment is a thread with a root message and collapsible replies.
- **CMT-003:** Users must add replies, edit/delete individual messages, resolve, reopen, and filter active-document threads by resolved state.
- **CMT-004:** Comment bodies are plain text with paragraph breaks.
- **CMT-005:** Comments appear in the active document’s Inspector panel and as editor highlights/decorations.
...
- **CARD-003:** Descendants are indented; groups expand/collapse; a visible insertion marker shows drag destinations.
...
- **CARD-006:** Card title, Synopsis, and project-configured visible metadata values are editable as permitted by the product model.
- **CARD-007:** Explorer and Inspector remain available around Cards.
- **CARD-008:** Activating a document from Cards switches to Editor and opens it; group interactions expand/collapse or focus the subtree as defined by the approved design.
- **CARD-009:** Cards and Explorer share multi-selection and applicable move/copy/cut behavior.

### 8.11 Search and replacement

- **SEARCH-001:** `Find` opens an animated local search bar in the focused editor view.
- **SEARCH-002:** Each view has independent local-search state.
- **SEARCH-003:** Enter/Shift+Enter navigate; Escape closes and returns focus; Replace expands local replacement controls.
- **SEARCH-004:** Local search supports case-sensitive and whole-word options and highlights all matches with an active-match distinction.
- **SEARCH-005:** Local replacement participates in document undo.
- **SEARCH-006:** Global Search replaces Explorer contents in the left sidebar and provides an explicit return to Explorer.
...
- **SEARCH-010:** Results stream, are virtualized, are grouped by document, and navigate to the matched content.
- **SEARCH-011:** Global replacement modifies editable document bodies only.
- **SEARCH-012:** Global replacement requires a central preview showing every proposed change, with per-match and per-document deselection.
...
- **HIST-007:** History must support chronological virtualized browsing, filtering to changes affecting the active document, preview, current comparison, document restore, group-subtree restore, and whole-project restore.
...
- **DEL-004:** Recently Deleted lists deleted documents and groups and can restore complete subtrees.
...
- **SPELL-001:** Spellcheck is a conditional v1 feature. It ships only if all three platform webviews support correct, performant behavior with project and global dictionaries.
- **SPELL-002:** If shipped, it must support project default language, optional per-document override, global dictionary, project dictionary, context suggestions, and viewport/recent-change bounded checking.
- **SPELL-003:** Failure to satisfy the spellcheck gate may defer spellcheck without blocking other v1 features; this must be recorded, not silently omitted.
