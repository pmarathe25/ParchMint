...
- **HIST-007:** History presents a virtualized writing timeline with persisted UTC date/session headings, visible named-milestone creation and active-document filtering, side-by-side checkpoint-versus-current comparison, and restoration of the entire project. When a selected checkpoint comparison is loaded, its timeline row shows the compact signed current-word delta; it never triggers an additional load. Older projects without timestamp metadata remain in an explicitly undated legacy group. Overflow comparisons use a bounded diff so a large manuscript never blocks the UI. Partial checkpoint restoration is deferred.
