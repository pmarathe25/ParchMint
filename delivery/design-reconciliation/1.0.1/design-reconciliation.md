...
**Manifest checksum:** `8e1d2ff8763a09cb457a64a290b826125d984fada5241ae4512425c5ee5b210f`

## 1. Validation result

- Manifest/schema: parsed as schema version 2; status is draft and governing versions match the dispatch.
- All paths/checksums: passed `build-checksums.py --verify`; the candidate manifest SHA-256 matches `approval.yaml`.
- `.penpot` source: checksum-valid user-exported archive from S10 repair run `20260804T230332Z-s10p`; this reconciliation used the frozen candidate package, not live Penpot.
...
- References: all 20 candidate baseline PNGs decode at 1440x900 and form ten complete Light/Dark pairs. The repaired dual-editor and recovery pairs are from the authoritative repair run; the Dark editor canvas is dark.
...
The complete 1.0.1 candidate retains the 79 component rows, 80 screen rows,
...
ISSUE-001 through ISSUE-005 and the two visual-reference mismatches are
resolved in `open-issues.yaml`. Source/font/reference deviations and unproven
native behavior remain nonblocking evidence limits. This reconciliation remains
pending until the product owner approves this exact checksum-valid candidate.
