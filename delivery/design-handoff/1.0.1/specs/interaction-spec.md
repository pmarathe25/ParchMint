...
- **CMT-001/002/003/004/010/011 — Comment threads.** The default shows all
  threads in one continuous, unsectioned list. `comments-unresolved-thread`
  shows an unresolved collapsed thread; `comments-replies-collapsed` collapses
  replies; `comments-replies-expanded-composer` shows replies in chronological
  order with the composer; `comments-resolved-thread` is a resolved-only filter
  that narrows that same list and never creates resolved/unresolved sections.
...
- **EXP-001/008 — Export scope.** `export-entire-manuscript` fixes Entire Manuscript
  as v1 scope: output path/name, title/page-break controls, numbering,
