...
## 1. Theme-reference layer plugin hashes vs packaged reference PNGs
...
- **What differs:** the `pm.reference.sha256` recorded on each locked layer and
  the `pm.theme-reference-checksums` block in page-15 plugin metadata were
  computed against the image snapshot at the time the references were shared.
  The packaged PNGs inside `ParchMint.penpot` (and exported to
  `references/light|dark/`) are from a slightly newer export, so their SHA-256
  values differ from the recorded values. `editor-dual-dark.png` additionally
  has a layer-plugin hash (`d96f4fe51cc8e9e4...`) that differs from its
  page-15 manifest entry (`f6bfebf47794c1ea...`).
